    ...
    <section class="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div class="glass-card p-6 rounded-3xl col-span-1 md:col-span-2 relative overflow-hidden bg-primary/5 border border-primary/10">
            <div class="relative z-10">
                <p class="text-primary font-bold text-sm mb-2">Persentase Kehadiran</p>
                <h2 class="text-4xl font-extrabold text-on-surface mb-2">{{ $attendanceRate }}%</h2>
                <p class="text-sm text-on-surface-variant">Luar biasa! Pertahankan konsistensi kehadiran Anda di setiap kelas.</p>
                <div class="mt-6 flex gap-4">
                    <span class="px-4 py-2 bg-success/10 text-success rounded-full text-xs font-semibold flex items-center gap-2">
                        <span class="material-symbols-outlined text-sm">trending_up</span> Di atas rata-rata kelas
                    </span>
                </div>
            </div>
        ...
        <div class="glass-card p-6 rounded-3xl flex flex-col justify-between bg-white border border-slate-200/50">
            <div>
                ...
                <p class="text-on-surface-variant font-bold text-sm">Hari Hadir</p>
                <h3 class="text-2xl font-extrabold">{{ $presentDays }} Hari</h3>
            </div>
            <p class="text-xs text-outline mt-2 italic">Kehadiran terkonfirmasi otomatis</p>
        </div>
        ...
        <div class="glass-card p-6 rounded-3xl flex flex-col justify-between bg-white border border-slate-200/50">
            <div>
                ...
                <p class="text-on-surface-variant font-bold text-sm">Ketidakhadiran (Alpha)</p>
                <h3 class="text-2xl font-extrabold">{{ $absentDays }} Hari</h3>
            </div>
            <p class="text-xs text-outline mt-2 italic">Hubungi wali kelas jika ada kekeliruan</p>
        </div>
    </section>
    ...
    <section class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="glass-card p-8 rounded-[32px] lg:col-span-2 bg-white border border-slate-200/50">
            <div class="flex items-center justify-between mb-8">
                <div>
                    <h3 class="text-xl font-bold">Kalender Presensi</h3>
                    <p class="text-on-surface-variant text-sm mt-1">Pemantauan kehadiran belajar Anda pada bulan {{ $currentMonthName }}</p>
                </div>
                <div class="flex gap-2">
                    <button class="px-4 py-2 bg-slate-50 hover:bg-slate-100 rounded-lg border border-outline-variant/30 text-sm font-semibold transition-colors">
                        {{ $currentMonthName }}
                    </button>
                </div>
            </div>
            ...
            <div class="grid grid-cols-7 gap-4 mb-8">
                ...
                @foreach($calendarDays as $cDay)
                    @if($cDay['status'] === 'blank')
                        <div class="h-16 rounded-2xl bg-slate-50/50 border border-transparent"></div>
                    @else
                        @php
        ...
        <div class="glass-card p-8 rounded-[32px] flex flex-col bg-white border border-slate-200/50">
            <h3 class="text-xl font-bold mb-6">Log Aktivitas Terbaru</h3>
            <div class="flex-1 space-y-8 relative before:absolute before:left-[11px] before:top-2 before:bottom-2 before:w-[2px] before:bg-slate-100">
                @forelse($absensis as $absen)
                    <div class="relative pl-10">
                        ...
                        @endif
                        <p class="font-bold text-sm text-on-surface">Status: {{ $absen->status }}</p>
                ...
                @empty
                    <div class="relative pl-10">
                        ...
                        <p class="font-bold text-sm text-slate-400">Belum Ada Aktivitas</p>
                        <p class="text-xs text-outline">Semua data kehadiran akan tercatat di sini.</p>
                    </div>
                @endforelse
            </div>
        </div>
