<div class="w-full space-y-8 text-left">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
            <h2 class="text-3xl md:text-5xl font-extrabold text-on-surface mb-2">Portal Pembayaran Sekolah</h2>
            <p class="text-lg text-on-surface-variant">Lacak kewajiban pembayaran SPP dan uang kegiatan anak Anda secara aman dan mudah.</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="glass-card rounded-3xl p-8 relative overflow-hidden bg-white border border-slate-200/50">
            ...
            <div class="relative z-10">
                <p class="text-xs font-bold text-outline uppercase tracking-wider mb-1">Total Tagihan Belum Lunas</p>
                <h3 class="text-4xl font-extrabold text-on-surface mb-2">Rp {{ number_format($totalTagihan, 0, ',', '.') }}</h3>
                <p class="text-xs text-on-surface-variant font-semibold">Harap selesaikan kewajiban tepat waktu.</p>
            </div>
        ...
        <div class="glass-card rounded-3xl p-8 relative overflow-hidden bg-white border border-slate-200/50">
            ...
            <div class="relative z-10">
                ...
                <p class="text-success flex items-center gap-1 text-xs font-semibold">
                    ...
                    Seluruh pembayaran lunas tercatat
                </p>
            </div>
            <div class="absolute -right-4 -bottom-4 opacity-10">
    ...
    <div class="bg-white border border-slate-200/50 rounded-[32px] p-8 shadow-sm">
        <h3 class="text-xl font-bold mb-6">Daftar Invoice & Riwayat Pembayaran</h3>
                            ...
                            <td class="py-4 px-4 text-center">
                                ...
                                @elseif($p->status === 'Lunas')
                                    @if($p->bukti_pembayaran)
                                        <a href="/storage/{{ $p->bukti_pembayaran }}" target="_blank" class="inline-flex items-center gap-1.5 px-4 py-2 bg-success hover:bg-green-600 text-white rounded-xl text-xs font-bold shadow-md shadow-success/10 transition-all">
                                            <span class="material-symbols-outlined text-sm">download</span>
                                            Download Bukti
                                        </a>
                                    @else
                                        <button onclick="window.print()" class="inline-flex items-center gap-1.5 px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl text-xs font-bold transition-all">
